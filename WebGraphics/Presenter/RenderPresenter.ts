class RenderPresenter {
    ...
    private renderDiv: JQuery;
    private renderBB: boolean;
    ...
    constructor(appModel: AppModel, renderDiv: JQuery) {
        ...
        var startX = 0;
        var startY = 0;
        renderDiv.mousedown((e) => {
            isMouseDown = true;
            startX = e.clientX;
            startY = e.clientY;
        });
        renderDiv.mousemove((e) => {
            if (isMouseDown) {
                ...
                if (this.rotations[0] !== undefined) {
                    rotationMatrixXold.makeRotationY(-this.rotations[0].deltaX / 100);
                    rotationMatrixYold.makeRotationX(-this.rotations[0].deltaY / 100);
                    rotationMatrixYold.multiply(rotationMatrixXold);
                }

                this.rotations[0].deltaX = e.clientX - startX;
                this.rotations[0].deltaY = e.clientY - startY; 
                ...
                if (this.rotations[0] !== undefined) {
                    rotationMatrixX.makeRotationY(this.rotations[0].deltaX / 100);
                    rotationMatrixY.makeRotationX(this.rotations[0].deltaY / 100);
                ...
                for (var i = 0; i < this.geometries[0].length; i++)
                {
                    this.geometries[0][i].applyMatrix(rotationMatrixYold);
                }
        ...
        renderDiv.mouseup((e) => {
            isMouseDown = false;
            this.rotations[0].x = this.rotations[0].x + e.clientX - startX;
            this.rotations[0].y = this.rotations[0].y + e.clientY - startY;
            this.rotations[0].deltaX = 0;
            this.rotations[0].deltaY = 0;
    ...
    private Render(molekula: MolekulaModel, offset: number, scene: any, molekulaIndex: number): any {

        var rotationMatrix = new THREE.Matrix4();
        rotationMatrix.identity();
        if (this.rotations[molekulaIndex] !== undefined) {
            //rotationMatrix.makeRotationY(this.rotations[molekulaIndex].x / 100);
        }
    ...
    public get RenderBB(): boolean {
        return this.renderBB;
    }

    public set RenderBB(value: boolean) {
        this.renderBB = value;
        this.RenderAll();
    }
